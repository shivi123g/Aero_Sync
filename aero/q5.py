...
if submitted:
    try:
        ...
        query= '''SELECT u.name AS pilot_name,f.status,COUNT(*) AS flight_count
FROM assigned_to a
JOIN Crew c ON a.crew_id= c.crew_id JOIN Users u ON c.user_id=u.user_id
JOIN Flight f ON a.flight_id=f.flight_id
WHERE c.position='Pilot'
GROUP BY u.name,f.status
