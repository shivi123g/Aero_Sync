...
with st.form("search_form"):
    ...
    value="Crew members not assigned to any flight in the last x months",
    disabled=True
)
    #list_order = st.selectbox("Airport", ["IGI", "Descending"])
    
   
    x = st.number_input("Enter months", min_value=0, max_value=24, value=1, step=1)
...
if submitted:
    try:
        ...
        query= '''SELECT u.name AS crew_member
FROM Crew c
JOIN Users u ON c.user_id=u.user_id
WHERE c.crew_id NOT IN (
    SELECT a.crew_id
    FROM assigned_to a
    WHERE a.assigned_date>=DATE_SUB(CURDATE(),INTERVAL '''+str(x)+''' MONTH)
);
'''
